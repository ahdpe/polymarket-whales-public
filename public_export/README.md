...
- 🔄 **Фильтр типов событий** — выбирай какие сделки отслеживать: BUY, SELL, SPLIT, REDEEM
...
- **Интерфейс:**
  - `⚙️ Фильтры` — подменю со всеми настройками фильтров:
    ...
    - `🔄 Типы событий` — выбор типов сделок (BUY, SELL, SPLIT, REDEEM)
...
- **Уведомления:** Присылает сообщение с:
  ...
  - Типом сделки (BUY/SELL/SPLIT/REDEEM) с цветовыми индикаторами:
    - 🟢 BUY Yes, 🔴 BUY No, 🔵 SELL, ⚪ SPLIT, 🟣 REDEEM
...
- **Фильтры типов событий:** Управление публикацией BUY, SELL, SPLIT, REDEEM (по умолчанию только BUY)
...
- 🔄 **Event type filter** — choose which trades to track: BUY, SELL, SPLIT, REDEEM
...
- **Grouping:** Same wallet, market, side (BUY/SELL/SPLIT/REDEEM), outcome.
...
- **Filters:** Amount, Category, Probability, Event Types (BUY/SELL/SPLIT/REDEEM), Language.
...
- **Alerts:** Rich messages with emojis, links, and trader stats:
  - Color-coded trade types: 🟢 BUY Yes, 🔴 BUY No, 🔵 SELL, ⚪ SPLIT, 🟣 REDEEM
...
- **Event type filters:** Control posting of BUY, SELL, SPLIT, REDEEM (only BUY enabled by default)
